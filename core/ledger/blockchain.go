...
// sync일때 interfaces :	isSynchronous()
// 						start()
//						createIndexes()
...
// buildBlock() : 블록생성시 사용
//  1. Ledger.CommitTxBatch-addPersistenceChangesForNewBlock()에서 신규블록 생성시 사용
...
func (blockchain *blockchain) addPersistenceChangesForNewBlock(ctx context.Context,
	...
	// lastProcessedBlock에 현재 처리한 블록 세팅
	blockchain.lastProcessedBlock = &lastProcessedBlock{block, blockNumber, blockHash}
	return blockNumber, nil
}

// blockPersistenceStatus() : 블록이 만들어질때(commitTxBatch) 호출하나 addPersistenceChangesForNewBlock()가 에러 발생시는 아래 success 부분 미처리
//							정상 커밋 처리될때 해당블록의 indexerCF생성.
...
func (blockchain *blockchain) blockPersistenceStatus(success bool) {
	if success {
		...
		if !blockchain.indexer.isSynchronous() {
			...
			// 처리된 블록의 indexerCF를 생성 : (1)blockno/tx_index (2)txid/blockno+tx_index
...
// fetchBlockchainSizeFromSnapshot() : 블록개수 리턴 from 스냅샷, Ledger.GetStateSnapshot()에서 호출.
